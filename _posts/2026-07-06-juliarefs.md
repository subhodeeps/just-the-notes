---
title: Julia Resources
description: "A collection of references, tutorials, and links for scientific computing using Julia."
date: 2026-07-06 00:00:00 +0530
categories: [Programming, Julia]
tags: [julia, computational-physics, numerical-methods]
math: true
---

## Julia Fundamentals

- [Video Tutorials by **`doggo dot jl`**](https://www.youtube.com/@doggodotjl)
- [Introduction to programming in Julia, ENCCS](https://enccs.github.io/julia-intro/)
- [Julia for Pythonistas](https://github.com/ageron/julia_notebooks)

## Books: Julia Language

- [Think Julia, Allen B. Downey (O’Reilly, 2019)](https://benlauwens.github.io/ThinkJulia.jl/latest/book.html)
- [Julia as a Second Language: General Purpose Programming With a Taste of Data Science, Erik Engheim, (Manning, 2023)](https://github.com/ordovician/code-samples-julia-second-language)

## Scientific Computing: Basic

- [Introduction to Computational Thinking, MIT](https://computationalthinking.mit.edu/Fall24/)

## Scientific Computing: Advanced

- [Solving PDEs in parallel on GPUs with Julia, ETH](https://pde-on-gpu.vaw.ethz.ch/)
- [Julia for high-performance scientific computing, ENCCS](https://enccs.github.io/julia-for-hpc/)
- Parallel Computing and Scientific Machine Learning (SciML): Methods and Applications, [SciML Book](https://book.sciml.ai/), [SciML Lectures](https://youtube.com/playlist?list=PLCAl7tjCwWyGjdzOOnlbGnVNZk0kB8VSa&si=FL1h-bzr2QL8X2fM)

## Books: Julia and Scientific Computing

- [Numerical Methods for Scientific Computing: The Definitive Manual for Math Geeks, Kyle A. Novak](https://www.equalsharepress.com/)
- [Fundamentals of Numerical Computation: Julia Edition, Tobin A. Driscoll & Richard J. Braun](https://tobydriscoll.net/fnc-julia/home.html). [Git Repo](https://github.com/fncbook/FundamentalsNumericalComputation.jl)
- [Algorithms with JULIA: Optimization, Machine Learning, and Differential Equations Using the JULIA Language, Clemens Heitzinger](http://www.heitzinger.info/Book-Julia.html)

## Machine Learning

- [Julia for Machine Learning, TU Berlin](https://adrianhill.de/julia-ml-course/)
