---
title: "Setting Up a Scientific Python Environment using <code>venv</code>"
description: "A guide to creating a reproducible scientific Python environment on macOS (or Linux)."
date: 2026-07-06 00:00:00 +0530
categories: [Programming, Tutorials]
tags: [python, macos, environment-setup]
math: false
pin: false
---
...
- You are running macOS on Apple Silicon (M1/M2/M3) or a Linux based OS
- You Python installed via Homebrew
- You have a terminal application available
...
**Note on multiple installations:** If `which python3` does not point to `/opt/homebrew/bin/python3`, or `python3 --version` does not show you the desired version number, you likely have multiple versions on your system. Run `which -a python3` to see all locations, or use `ls /opt/homebrew/bin/ | grep python` to find the specific Homebrew-managed command. To avoid ambiguity, you can invoke the correct version directly by its full name (e.g., `python3.14`).
...
Some packages in the scientific stack (like `h5py`) require C-based system libraries to be present on the machine. Before installing the requirements, run:
